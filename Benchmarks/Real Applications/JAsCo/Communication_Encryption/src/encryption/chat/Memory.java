...
public class Memory {
	...
	public static int getPeakMemoryUsage(String process) {
		String host = "localhost"; // Technically you should be able to connect
									// to other hosts, but it takes setup
		String connectStr = String.format("winmgmts:\\\\%s\\root\\CIMV2", host);
		String query = "Select * from Win32_Process"; // Started = 1 means the
														// service is running.
		ActiveXComponent axWMI = new ActiveXComponent(connectStr);
		// Execute the query
		Variant vCollection = axWMI.invoke("ExecQuery",
				new Variant[] { new Variant(query) });
		...
		while (enumVariant.hasMoreElements()) {
			...
			if (serviceName.equalsIgnoreCase(process)) {
				String commandLine = Dispatch.call(item, "CommandLine")
						.toString();
				System.out.println(commandLine);
				peakMemorySize = Dispatch.call(item, "PeakWorkingSetSize")
						.getInt();
