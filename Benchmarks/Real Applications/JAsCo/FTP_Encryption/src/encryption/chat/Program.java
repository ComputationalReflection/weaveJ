...
public class Program {
	...
	private void ReceiveMessages(String i) throws IOException {
		...
		BufferedWriter out = new BufferedWriter(new FileWriter(
				"TheCountOfMonteCristo_copy" + i + ".txt"));
	...
	public static void main(String[] args) throws NumberFormatException,
		...
		// System.out.println("Push a key");
		// System.in.read();
		int maxIterations = steady ? 30 : 1, k = steady ? 10 : 1, minIter = steady ? 15
				: 1;
		// double CoV = 0.02;
		...
		for (int j = 1; j < minIter || j <= maxIterations
				&& !areWeDone(executionTimes, k, CoV); j++) {
			...
			System.out.println("The duration without aspects is.... "
					+ (end - now));
			now = System.currentTimeMillis();
			if (args.length == 1) {
			...
			System.out.println("The duration with aspect is  .... "
					+ (end - now));
			now = System.currentTimeMillis();
			Thread.sleep(1500);
			program.run2(args.length);
			...
			System.out.println("The duration without aspect is  .... "
					+ (end - now));
