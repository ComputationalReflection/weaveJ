...
public class Program {
	...
	public static void main(String[] args) throws Throwable {
		...
		// System.in.read();
		int maxIterations = steady ? 30 : 1, k = steady ? 8 : 1, minIter = steady ? 8 : 1;
		// double CoV = 0.02;
	...
	public static void mainSteady(String[] args) throws Throwable {
		System.out.println("NEW VSTEAD:");
		long totalNow = 0, now = 0, end = 0, totalDuration = 0, totalDurationTotal = 0;
		totalNow = now = System.currentTimeMillis();
		Program program = new Program();
		if (args.length == 1)
			program.ConnectAsServer(args[0]);
		...
		// System.out.println("Push a key");
		// System.in.read();
		int maxIterations = 30, k = 8, minIter = 8;
		double CoV = 0.02;
		List<Double> executionTimes = new ArrayList<Double>();
		for (int i = 1; i < minIter || i <= maxIterations && !areWeDone(executionTimes, k, CoV); i++) {
			System.out.println("Iter " + i);
			totalNow = System.currentTimeMillis();
			program.run2(args.length);
			end = System.currentTimeMillis();
			totalDuration = end - now;
			totalDurationTotal += totalDuration;
			System.out.println("The duration without aspects is.... " + totalDuration);
			now = System.currentTimeMillis();
			Pointcut pointcut = args.length == 1
					? Weaver.weaveAspectForMethodAround("encryption.chat.Program", "getSrReceiver",
							"encryption.aspect.ChangeServerCommunicationAspect", "getSrReceiverAround",
			...
			totalDuration = end - now;
			totalDurationTotal += totalDuration;
			System.out.println("The duration with aspect is  .... " + totalDuration);
			now = System.currentTimeMillis();
			Thread.sleep(1500);
			program.run2(args.length);
			end = System.currentTimeMillis();
		...
		if (args.length == 1) {
			...
			bw.append(("\n" + getMean(executionTimes, k)).replace(".", ","));
			bw.close();
		}
		System.out.println("The duration without aspect is  .... " + totalDuration);
